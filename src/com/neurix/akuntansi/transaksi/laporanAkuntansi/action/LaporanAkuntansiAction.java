...
//        listNeracaSaldo = laporanAkuntansiBo.getListNeracaSaldo(reportId,data.getBulan()+"-"+data.getTahun(),unit);
        ...
        reportParams.put("periodeLalu", Integer.valueOf(data.getBulan()) - 1 + "-" + data.getTahun());
        reportParams.put("kota", branch.getBranchName());
        reportParams.put("alamatSurat", branch.getAlamatSurat());
        reportParams.put("reportId", data.getReportId());
...
//        listBukuBesar = laporanAkuntansiBo.getListKartuBukuBesar(reportId,data.getBulan()+"-"+data.getTahun(),unit,data.getKodeRekening(),data.getMasterId(),data.getTipeTanggal(),tanggalAwal,tanggalAkhir);
        ...
        *//*if (listBukuBesar.size()==0){
        ...
        reportParams.put("kd_barang", data.getKodeBarang().isEmpty() ? "%" : data.getKodeBarang());
        reportParams.put("kode_rekening", data.getKodeRekening());
        reportParams.put("vendor_id",data.getMasterId().isEmpty() ? "%" : data.getMasterId());
        reportParams.put("branchId", unit);
        reportParams.put("areaId", CommonUtil.userAreaName());
        reportParams.put("kota",branch.getBranchName());
        reportParams.put("urlLogo", CommonConstant.URL_LOGO_REPORT + branch.getLogoName());
        ...
        if (("T").equalsIgnoreCase(data.getTipeTanggal())) {
            ...
            c.setTime(tanggalAwal);
            if(!stTanggalAwalBulan[0].equals("01")){
            ...
            reportParams.put("tanggalAkhirBaru", tanggalAkhirBaru);
            reportParams.put("tanggalAkhir", tanggalAkhir);
            reportParams.put("periodeTitle", data.getStTanggalAwal() + " s/d " + data.getStTanggalAkhir());

            if (!("").equalsIgnoreCase(data.getMasterId()) || !("").equalsIgnoreCase(data.getKodeBarang())) {
...
//        System.out.println(CommonConstant.URL_LOGO_REPORT+branch.getLogoName());
    ...
    public String printReportRekapClosingKasir() {
        logger.info("[LaporanAkuntansiAction.printReportRekapClosingKasir] start process >>>");
        ApplicationContext ctx = ContextLoader.getCurrentWebApplicationContext();
        LaporanAkuntansiBo laporanAkuntansiBo = (LaporanAkuntansiBo) ctx.getBean("laporanAkuntansiBoProxy");
        ...
        reportParams.put("userId", data.getNip());
        reportParams.put("tipeKasir", data.getTipeKasir());
        reportParams.put("reportId", data.getReportId());
        ...
        try {
            ...
            try {
                ...
                logger.error("[LaporanAkuntansiAction.printReportRekapClosingKasir] Error when downloading ,", e1);
            }
            logger.error("[LaporanAkuntansiAction.printReportRekapClosingKasir] Error when print report ," + "[" + logId + "] Found problem when downloading data, please inform to your admin.", e);
            addActionError("Error, " + "[code=" + logId + "] Found problem when downloading data, please inform to your admin.");
        }
        logger.info("[LaporanAkuntansiAction.printReportRekapClosingKasir] end process <<<");
        return "print_report_rekap_closing_kasir";
    }
    ...
    public String printReportClosingKasir() {
        ...
        Branch branch = branchBo.getBranchById(data.getUnit(), "Y");
        Biodata biodata = biodataBo.getBiodataByNip(data.getNip());
        String titleReport = "LAPORAN CLOSING KASIR HARIAN";
        java.sql.Date tglFrom = CommonUtil.convertStringToDate(data.getStTanggalAwal());
        ...
        try {
            ...
            try {
                ...
                logger.error("[LaporanAkuntansiAction.printReportClosingKasir] Error when downloading ,", e1);
            }
            logger.error("[LaporanAkuntansiAction.printReportClosingKasir] Error when print report ," + "[" + logId + "] Found problem when downloading data, please inform to your admin.", e);
            addActionError("Error, " + "[code=" + logId + "] Found problem when downloading data, please inform to your admin.");
        }
...
//        String reportId = "RPT23";
    ...
    public String printReportAging() {
        ...
        String titleReport = "";
        String reportId=dataLaporan.getReportId();
//        String reportId = "RPT07";
        String tipeAging = "";
        String unit = "";
        if ((CommonConstant.BRANCH_KP).equalsIgnoreCase(dataLaporan.getUnit())) {
            List<Branch> branchList = new ArrayList<>();
            branchList = branchBo.getAll();
            int i = 1;
            for (Branch dataUnit : branchList) {
        ...
        List<Aging> listOfAgingResult = new ArrayList<>();
        if (agingList.size() == 0) {
        ...
        if (agingList.size() > 0) {
            ...
            if (listOfAgingTemp.size() > 0) {
                for (Aging aging : listOfAgingTemp) {
                    String nota = aging.getNoNota();
                    String noJurnal = aging.getNoJurnal();
                    List<Aging> listLawan = laporanAkuntansiBo.getLawanAging(tipeAging, branch.getBranchId(), noJurnal, nota);
                    if (listLawan.size() > 0) {
                        BigDecimal sisaAging = aging.getTotal();
                        for (Aging pelunasanAging : listLawan) {
                            sisaAging = sisaAging.add(pelunasanAging.getTotal());
                        }
                        if (sisaAging.compareTo(BigDecimal.ZERO) != 0) {
                            listOfAgingResult.add(aging);
                        }
                    } else {
                        listOfAgingResult.add(aging);
                    }
        ...
        if (listOfAgingResult.size() > 0) {
            myList = new ArrayList();
            for (Aging data : listOfAgingResult) {
...
//            myList.addAll(forReport);
    ...
    public boolean cekIsObjectIsDoubel(Aging data, List<Aging> listData) {
        ...
        for (int i = 0; i < listData.size() && !flag; i++) {
            ...
            if (aging.getNoNota() != null && aging.getNoNota().equalsIgnoreCase(data.getNoNota())
                    && aging.getMataUang() != null && aging.getMataUang().equalsIgnoreCase(data.getMataUang())
                    && aging.getMasterId() != null && aging.getMasterId().equalsIgnoreCase(data.getMasterId())) {
                flag = true;
            }
        }
        return flag;
    ...
    public List<JadwalShiftKerjaDetail> listKasirByBranch(String branchId, String stTglFrom, String tipeKasir) {
        ...
        try {
            detailList = jadwalShiftKerjaBo.getJadwalShiftKerjaByUnitAndProfesiAndTanggal(branchId, tglFrom, tglFrom, tipeKasir);
        } catch (GeneralBOException e) {
            Long logId = null;
            try {
