...
/**
 * updated by Ferdi on 01/12/2020.
 */

public class PayrollAction extends BaseTransactionAction {
...
//    private List<Payroll> payrollList = new ArrayList<>();
    ...
    private String idPayrollHeader;
    ...
    public void setApprovedSdm(String approvedSdm) {
        this.approvedSdm = approvedSdm;
    }

    public String getApprovedAks() {
        return approvedAks;
    }

    public void setApprovedAks(String approvedAks) {
        this.approvedAks = approvedAks;
    }
    ...
    public void setEditSearch(boolean editSearch) {
        this.editSearch = editSearch;
    }

    public String getIdPayrollHeader() {
        return idPayrollHeader;
    }

    public void setIdPayrollHeader(String idPayrollHeader) {
        this.idPayrollHeader = idPayrollHeader;
    }
...
//
    ...
    //updated by ferdi, 01-12-2020
    public void initComboPeriodeTahunKurang10() {
    ...
    public void initComboPeriodeTahunKurang5() {
    ...
    //updated by ferdi, 01-12-2020, init form payroll
    @Override
    public String initForm() {
        ...
        if (branchId!=null) {
            ...
            payrollHeader.setBranchId("");
        }

        if (CommonConstant.ROLE_ID_ADMIN_SDM.equalsIgnoreCase(userRoleId)){
            payrollHeader.setSdm(true);
        } else {
            payrollHeader.setSdm(false);
        }

        setPayrollHeader(payrollHeader);
    ...
    //updated by ferdi, 01-12-2020, call first page for adding payroll
    public String add() {
        ...
        HttpSession session = ServletActionContext.getRequest().getSession();
        List<PegawaiPayroll> listOfPegawaiPayroll = (List<PegawaiPayroll>) session.getAttribute("listDataPayroll");
        if (session.getAttribute("listDataPayroll") != null) {
            if (!listOfPegawaiPayroll.isEmpty()) {
                payrollHeader.setAfterSave(true);
            } else {
                payrollHeader.setAfterSave(false);
            }
    ...
    //updated by ferdi, 01-12-2020, gunakan payroll header saat search
    @Override
    public String search() {
        logger.info("[PayrollAction.search] start process >>>");

        PayrollHeader searchPayroll = getPayrollHeader();
        List<PayrollHeader> listOfsearchPayroll = new ArrayList<>();

        try {

            listOfsearchPayroll = payrollBoProxy.searchCalculatePayroll(searchPayroll);

        } catch (GeneralBOException e) {
            Long logId = null;
            try {
                logId = payrollBoProxy.saveErrorMessage(e.getMessage(), "PayrollBO.searchCalculatePayroll");
            } catch (GeneralBOException e1) {
                logger.error("[PayrollAction.search] Error when saving error,", e1);
                return ERROR;
            }
            logger.error("[PayrollAction.search] Error when searching initial payroll," + "[" + logId + "] Found problem when searching data initial payroll, please inform to your admin.", e);
            addActionError("Error, " + e.getMessage() + "[code=" + logId + "] Found problem when searching data initial payroll, please inform to your admin" );
            return ERROR;
        }
        ...
        if (CommonUtil.roleIdAsLogin().equalsIgnoreCase(CommonConstant.ROLE_ID_ADMIN_AKS)){
            searchPayroll.setKeuangan(true);
        }

        if (CommonUtil.roleIdAsLogin().equalsIgnoreCase(CommonConstant.ROLE_ID_ADMIN_SDM)){
...
//        payroll=searchPayroll;
    ...
    //updated by ferdi, 01-12-2020,to view only

    public String view() {
        logger.info("[PayrollAction.view] start process >>>");

        PayrollHeader viewPayrollHeader = new PayrollHeader();
        ...
        if (!editSearch) {
            ...
            String tipePayroll = getTipePayroll();
            String approvedSdm = getApprovedSdm();
            String approvedAks = getApprovedAks();

            viewPayrollHeader.setPayrollHeaderId(payrollHeaderId);
            viewPayrollHeader.setBranchId(branchId);
            viewPayrollHeader.setBulan(bulan);
            viewPayrollHeader.setTahun(tahun);
            viewPayrollHeader.setTipePayroll(tipePayroll);
            viewPayrollHeader.setCreatedWho(userLogin);
            viewPayrollHeader.setApprovalAksFlag(approvedAks);
            viewPayrollHeader.setApprovalSdmFlag(approvedSdm);

            if (CommonConstant.BULAN_12.equalsIgnoreCase(bulan) && CommonConstant.CODE_PAYROLL.equalsIgnoreCase(tipePayroll)) {
                viewPayrollHeader.setEditPphBulan12(true);
            } else {
                viewPayrollHeader.setEditPphBulan12(false);
            }

            List<PegawaiPayroll> listOfPegawaiPayrollEdit = new ArrayList<>();

            try {

                listOfPegawaiPayrollEdit = payrollBoProxy.viewAllCalculatePayroll(viewPayrollHeader);

            } catch (GeneralBOException e) {
                Long logId = null;
                try {
                    logId = payrollBoProxy.saveErrorMessage(e.getMessage(), "PayrollBO.viewAllCalculatePayroll");
                } catch (GeneralBOException e1) {
                    logger.error("[PayrollAction.view] Error when saving error,", e1);
                    return ERROR;
                }
                logger.error("[PayrollAction.view] Error when getting view saved payroll," + "[" + logId + "] Found problem when getting view data saved payroll, please inform to your admin.", e);
                addActionError("Error, " + e.getMessage() + "[code=" + logId + "] Found problem when getting view data saved payroll, please inform to your admin" );
                return ERROR;
            }
            ...
            setPayrollHeader(viewPayrollHeader);

        } else {

            PayrollHeader viewedPayrollHeader = getPayrollHeader();
            String nip = viewedPayrollHeader.getNip();
            HttpSession session = ServletActionContext.getRequest().getSession();
            ...
            if (sessionListPegawaiPayroll!=null) {
                ...
                List<PegawaiPayroll> listOfPegawaiPayrolView = sessionListPegawaiPayroll.stream().filter(
                        p -> p.getNip().equalsIgnoreCase(nip)
                ).collect(Collectors.toList());

                session.removeAttribute("listDataPayroll");
                session.setAttribute("listDataPayroll", listOfPegawaiPayrolView);

            }

        }

        logger.info("[payrollAction.view] end process <<<");

        return "init_view";
    }


    //updated by ferdi, 01-12-2020, save intial payroll
    public String saveAdd(){
        logger.info("[PayrollAction.saveAdd] start process >>>");

        PayrollHeader payrollHeader = getPayrollHeader();
        String userLogin = CommonUtil.userLogin();
        payrollHeader.setCreatedWho(userLogin);
        List<PegawaiPayroll> listOfPegawaiPayroll = new ArrayList<>();

        try {

            listOfPegawaiPayroll = payrollBoProxy.saveInitialCalculatePayroll(payrollHeader);

        } catch (GeneralBOException e) {
            Long logId = null;
            try {
                logId = payrollBoProxy.saveErrorMessage(e.getMessage(), "PayrollBO.saveInitialCalculatePayroll");
            } catch (GeneralBOException e1) {
                logger.error("[PayrollAction.saveAdd] Error when saving error,", e1);
                return ERROR;
            }
            logger.error("[PayrollAction.saveAdd] Error when saving initial payroll," + "[" + logId + "] Found problem when saving data initial payroll, please inform to your admin.", e);
            addActionError("Error, " + e.getMessage() + ",[code=" + logId + "] Found problem when saving data initial payroll, please inform to your admin" );
            return ERROR;
        }
        ...
        //set to session list of pegawai payroll
        session.setAttribute("listDataPayroll",listOfPegawaiPayroll);
    ...
    //updated by ferdi, 01-12-2020, to approve aks, can approve = Y or N
    public String approvePayrollKeu(String payrollHeaderId, String branchId, String bulan, String tahun, String statusApproval, String tipe, String keterangan) {
        logger.info("[PayrollAction.approvePayrollKeu] start process >>>");

        boolean flag = true;
        String userLogin = CommonUtil.userLogin();

        PayrollHeader approvalKeuPayroll = new PayrollHeader();
        approvalKeuPayroll.setPayrollHeaderId(payrollHeaderId);
        approvalKeuPayroll.setBranchId(branchId);
        approvalKeuPayroll.setBulan(bulan);
        approvalKeuPayroll.setTahun(tahun);
        approvalKeuPayroll.setTipePayroll(tipe);
        approvalKeuPayroll.setCreatedWho(userLogin);
        approvalKeuPayroll.setApprovalAksFlag(statusApproval);
        approvalKeuPayroll.setKeteranganAks(keterangan);

        ApplicationContext ctx = ContextLoader.getCurrentWebApplicationContext();
        PayrollBo payrollBo = (PayrollBo) ctx.getBean("payrollBoProxy");

        try {
            payrollBo.savePostingAllCalculatePayroll(approvalKeuPayroll);
        } catch (GeneralBOException e) {
            flag = false;
            Long logId = null;
            try {
                logId = payrollBo.saveErrorMessage(e.getMessage(), "PayrollAction.approvePayrollKeu");
            } catch (GeneralBOException e1) {
                logger.error("[PayrollAction.approvePayrollKeu] Error when saving error,", e1);
            }
            logger.error("[PayrollAction.approvePayrollKeu] Error when saving update approval payroll by keuangan," + "[" + logId + "] Found problem when saving update payroll by keuangan, please inform to your admin.", e);
        }

        logger.info("[PayrollAction.approvePayrollKeu] end process <<<");
    ...
    //updated by ferdi, 01-12-2020, to approve sdm, should be approve = Y
    public String approvePayrollSdm(String payrollHeaderId, String branchId, String bulan, String tahun, String tipe) {
        logger.info("[PayrollAction.approvePayrollSdm] start process >>>");

        boolean flag = true;
        String userLogin = CommonUtil.userLogin();

        PayrollHeader approvalPayroll = new PayrollHeader();
        approvalPayroll.setPayrollHeaderId(payrollHeaderId);
        approvalPayroll.setBranchId(branchId);
        approvalPayroll.setBulan(bulan);
        approvalPayroll.setTahun(tahun);
        approvalPayroll.setTipePayroll(tipe);
        approvalPayroll.setCreatedWho(userLogin);

        ApplicationContext ctx = ContextLoader.getCurrentWebApplicationContext();
        PayrollBo payrollBo = (PayrollBo) ctx.getBean("payrollBoProxy");

        try {
            payrollBo.saveApprovalSdmPayroll(approvalPayroll);
        } catch (GeneralBOException e) {
            flag = false;
            Long logId = null;
            try {
                logId = payrollBo.saveErrorMessage(e.getMessage(), "PayrollBo.saveApprovalSdmPayroll");
            } catch (GeneralBOException e1) {
                logger.error("[PayrollAction.approvePayrollSdm] Error when saving error,", e1);
            }
            logger.error("[PayrollAction.approvePayrollSdm] Error when saving update approval payroll by sdm," + "[" + logId + "] Found problem when saving update payroll by sdm, please inform to your admin.", e);
        }

        logger.info("[PayrollAction.approvePayrollSdm] end process <<<");
    ...
    //updated by ferdi, 01-12-2020, to get list of pegawai based on autocomplete
    public List initComboPersonil(String query, String branchId) {
        logger.info("[PayrollAction.initComboPersonil] start process >>>");
        ...
        try {
            ...
            Long logId = null;
            try {
                logId = biodataBo.saveErrorMessage(e.getMessage(), "biodataBo.getListOfPersonil");
            } catch (GeneralBOException e1) {
                logger.error("[PayrollAction.initComboPersonil] Error when getting list of pegawai,", e1);
            }
            logger.error("[PayrollAction.initComboPersonil] Error when getting list of pegawai ," + "[" + logId + "] Found problem when getting list of pegawai, please inform to your admin.", e);
        }

        logger.info("[PayrollAction.initComboPersonil] end process <<<");

        return listOfUser;
    }


    //updated by ferdi, 01-12-2020, initial edit payroll
    public String edit() {
        ...
        if (!editSearch) {
            ...
            if (payrollHeaderId==null) { //get form dari model (reload / reset)
                PayrollHeader formPayrollHeader = getPayrollHeader();
                payrollHeaderId = formPayrollHeader.getPayrollHeaderId();
                branchId = formPayrollHeader.getBranchId();
                bulan = formPayrollHeader.getBulan();
                tahun = formPayrollHeader.getTahun();
                tipePayroll = formPayrollHeader.getTipePayroll();

            }

            editPayrollHeader.setPayrollHeaderId(payrollHeaderId);
            editPayrollHeader.setBranchId(branchId);
            editPayrollHeader.setBulan(bulan);
            editPayrollHeader.setTahun(tahun);
            editPayrollHeader.setTipePayroll(tipePayroll);
            editPayrollHeader.setCreatedWho(userLogin);

            //set empty string for nama pegawai
            editPayrollHeader.setNip(null);
            editPayrollHeader.setPegawaiName(null);

            if (CommonConstant.BULAN_12.equalsIgnoreCase(bulan) && CommonConstant.CODE_PAYROLL.equalsIgnoreCase(tipePayroll)) {
                editPayrollHeader.setEditPphBulan12(true);
            } else {
                editPayrollHeader.setEditPphBulan12(false);
            }
            ...
            try {
                ...
                if (reloadData) {
                    listOfPegawaiPayrollEdit = payrollBoProxy.refreshAllCalculatePayroll(editPayrollHeader);
                } else {
                    listOfPegawaiPayrollEdit = payrollBoProxy.editAllCalculatePayroll(editPayrollHeader);
                }
                ...
                Long logId = null;
                try {
                    logId = payrollBoProxy.saveErrorMessage(e.getMessage(), "PayrollBO.editAllCalculatePayroll");
                } catch (GeneralBOException e1) {
                    logger.error("[PayrollAction.edit] Error when saving error,", e1);
                    return ERROR;
                }
                logger.error("[PayrollAction.edit] Error when getting edit initial payroll," + "[" + logId + "] Found problem when getting edit data initial payroll, please inform to your admin.", e);
                addActionError("Error, " + e.getMessage() + ",[code=" + logId + "] Found problem when getting edit data initial payroll, please inform to your admin");
                return ERROR;
            }
    ...
    //updated by ferdi, 01-12-2020, refresh all item payroll
    public String refresh() {
        ...
        PayrollHeader payrollHeader = getPayrollHeader();
        payrollHeader.setPayrollHeaderId(payrollHeaderId);
        payrollHeader.setBranchId(branchId);
        payrollHeader.setBulan(bulan);
        payrollHeader.setTahun(tahun);
        payrollHeader.setTipePayroll(tipePayroll);
        payrollHeader.setCreatedWho(userLogin);

        List<PegawaiPayroll> listOfPegawaiPayrollRefresh = new ArrayList<>();

        try {

            listOfPegawaiPayrollRefresh = payrollBoProxy.refreshAllCalculatePayroll(payrollHeader);

        } catch (GeneralBOException e) {
            Long logId = null;
            try {
                logId = payrollBoProxy.saveErrorMessage(e.getMessage(), "PayrollBO.refreshAllCalculatePayroll");
            } catch (GeneralBOException e1) {
                logger.error("[PayrollAction.edit] Error when saving error,", e1);
                return ERROR;
            }
            logger.error("[PayrollAction.edit] Error when getting refresh new initial payroll," + "[" + logId + "] Found problem when getting refresh new data initial payroll, please inform to your admin.", e);
            addActionError("Error, " + e.getMessage() + "[code=" + logId + "] Found problem when getting refresh new data initial payroll, please inform to your admin" );
            return ERROR;
        }

        HttpSession session = ServletActionContext.getRequest().getSession();
        session.removeAttribute("listDataPayroll");
        session.setAttribute("listDataPayroll", listOfPegawaiPayrollRefresh);

        logger.info("[payrollAction.refresh] end process <<<");

        return "init_edit";
    }

    //updated by ferdi, 01-12-2020, to edit item payroll
    public PegawaiPayroll getDetailEdit(String payrollId) {
        logger.info("[PayrollAction.getDetailEdit] start process >>>");
        ...
        logger.info("[payrollAction.getDetailEdit] end process <<<");

        return itemEditDataPayroll;
    }

    //updated by ferdi, 01-12-2020, to save edit item payroll, komponen yang bisa di edit adalah potongan (iuran) serta selisih pp21 saat bulan 12
    public String saveEditData(String payrollId, String nip, String bulan,  //data pegawai
        ...
        if (listOfDataPayroll!=null && !listOfDataPayroll.isEmpty()) {
            ...
            if (itemEditDataPayroll!=null) {
                ...
                itemEditDataPayroll.setKopkar(kopkar);
                itemEditDataPayroll.setKopkarNilai(BigDecimal.valueOf(Double.parseDouble(CommonUtil.removeCommaNumber(kopkar))));

                itemEditDataPayroll.setIuranSp(iuranSp);
                itemEditDataPayroll.setIuranSpNilai(BigDecimal.valueOf(Double.parseDouble(CommonUtil.removeCommaNumber(iuranSp))));

                itemEditDataPayroll.setIuranPiikb(iuranPiikb);
                itemEditDataPayroll.setIuranPiikbNilai(BigDecimal.valueOf(Double.parseDouble(CommonUtil.removeCommaNumber(iuranPiikb))));

                itemEditDataPayroll.setBankMandiri(bankMandiri);
                itemEditDataPayroll.setBankMandiriNilai(BigDecimal.valueOf(Double.parseDouble(CommonUtil.removeCommaNumber(bankMandiri))));

                itemEditDataPayroll.setBankBri(bankBri);
                itemEditDataPayroll.setBankBriNilai(BigDecimal.valueOf(Double.parseDouble(CommonUtil.removeCommaNumber(bankBri))));

                itemEditDataPayroll.setInfaq(infaq);
                itemEditDataPayroll.setInfaqNilai(BigDecimal.valueOf(Double.parseDouble(CommonUtil.removeCommaNumber(infaq))));

                itemEditDataPayroll.setPerkesDanObat(perkesDanObat);
                itemEditDataPayroll.setPerkesDanObatNilai(BigDecimal.valueOf(Double.parseDouble(CommonUtil.removeCommaNumber(perkesDanObat))));

                itemEditDataPayroll.setListrik(listrik);
                itemEditDataPayroll.setListrikNilai(BigDecimal.valueOf(Double.parseDouble(CommonUtil.removeCommaNumber(listrik))));

                itemEditDataPayroll.setIuranProfesi(iuranProfesi);
                itemEditDataPayroll.setIuranProfesiNilai(BigDecimal.valueOf(Double.parseDouble(CommonUtil.removeCommaNumber(iuranProfesi))));
                ...
                if (CommonConstant.BULAN_12.equalsIgnoreCase(bulan)) {
                    ...
                    itemEditPayrollPph.setSelisihPph(selisihPph21);
                    itemEditPayrollPph.setSelisihPphNilai(BigDecimal.valueOf(Double.parseDouble(CommonUtil.removeCommaNumber(selisihPph21))));
                }
        ...
        logger.info("[payrollAction.saveEditData] end process <<<");
    ...
    public List<PayrollPph> searchTotalPPh11Bulan(String payrollId , String tahun){
        ...
        logger.info("[PayrollAction.searchTotalPPh11Bulan] start process >>>");

        List<PayrollPph> listOfPayrollPphUntilNov = new ArrayList<>();
        ...
        if (listOfDataPayroll!=null && !listOfDataPayroll.isEmpty()) {
            ...
            if (itemEditDataPayroll!=null) {
                listOfPayrollPphUntilNov = itemEditDataPayroll.getListOfPphUntilNov();
            }
        }

        logger.info("[payrollAction.searchTotalPPh11Bulan] end process <<<");

        return listOfPayrollPphUntilNov;

    }


    //updated by ferdi, 01-12-2020, to view detail lembur per person
    public List<AbsensiPegawai> searchDetailLembur(String nip, String branchId, String bulan, String tahun) {

        logger.info("[PayrollAction.searchDetailLembur] start process >>>");

        List<AbsensiPegawai> absensiPegawaiList = new ArrayList<>();
        PegawaiPayroll itemViewDataPayroll = new PegawaiPayroll();
        itemViewDataPayroll.setNip(nip);
        itemViewDataPayroll.setBranchId(branchId);
        itemViewDataPayroll.setPeriodePayroll(bulan);
        itemViewDataPayroll.setTahunPayroll(tahun);

        ApplicationContext ctx = ContextLoader.getCurrentWebApplicationContext();
        PayrollBo payrollBo = (PayrollBo) ctx.getBean("payrollBoProxy");

        try {
            absensiPegawaiList = payrollBo.getListAbsensiLembur(itemViewDataPayroll);
        } catch (GeneralBOException e) {
            Long logId = null;
            try {
                logId = payrollBo.saveErrorMessage(e.getMessage(), "PayrollBo.getListAbsensiLembur");
            } catch (GeneralBOException e1) {
                logger.error("[PayrollAction.saveEditData] Error when getting list of absensi pegawai error,", e1);
            }
            logger.error("[PayrollAction.saveEditData] Error when getting list of absensi pegawai," + "[" + logId + "] Found problem when getting list of absensi pegawai, please inform to your admin.", e);
        }

        logger.info("[payrollAction.searchDetailLembur] end process <<<");
    ...
    public String cancelPage(){
        ...
        PayrollHeader searchPayroll = new PayrollHeader();
        ...
        setPayrollHeader(searchPayroll);
    ...
    public String printReportPayrollByBranch(){
        ...
        String payrollHeaderId = getIdPayrollHeader();
        String branchId = getBranchId();
        ...
        if(tipe.equalsIgnoreCase(CommonConstant.CODE_PAYROLL)){
            reportParams.put("titleReport", "Slip Gaji");
            hasil = "success_print_report_branch_payroll";
        }else if(tipe.equalsIgnoreCase(CommonConstant.CODE_CUTI_TAHUNAN)){
            reportParams.put("titleReport", "Slip Cuti Tahunan");
            hasil = "success_print_report_payroll_Cuti_tahunan_branch";
        }else if(tipe.equalsIgnoreCase(CommonConstant.CODE_THR)){
            reportParams.put("titleReport", "Slip THR");
            hasil = "success_print_report_payroll_thr_branch";
        }else if(tipe.equalsIgnoreCase(CommonConstant.CODE_INSENTIF)){
            reportParams.put("titleReport", "Slip Insentive");
            hasil = "success_print_report_payroll_insentif_branch";
        }else if(tipe.equalsIgnoreCase(CommonConstant.CODE_JASOP)){
            reportParams.put("titleReport", "Slip Jasa Operational");
            hasil = "success_print_report_payroll_jasprod_branch";
        }else if(tipe.equalsIgnoreCase(CommonConstant.CODE_CUTI_PANJANG)){
            reportParams.put("titleReport", "Slip Cuti Panjang");
            hasil = "success_print_report_payroll_Cuti_panjang_branch";
        }else if(tipe.equalsIgnoreCase(CommonConstant.CODE_PENSIUN)){
            reportParams.put("titleReport", "Slip Pensiun");
            hasil = "success_print_report_payroll_pensiun_branch";
        }else if(tipe.equalsIgnoreCase(CommonConstant.CODE_RAPEL)){
            reportParams.put("titleReport", "Slip Rapel");
            hasil = "success_print_report_payroll_rapel_branch";
        }else if(tipe.equalsIgnoreCase(CommonConstant.CODE_PMP)){
            reportParams.put("titleReport", "Slip PMP");
            hasil = "success_print_report_payroll_pmp_branch";
        }
        ...
        if (branchId != null) {
            ...
            try{
                ...
                logger.error("[ReportAction.printReportPayrollByBranch] Error when getting branch, please inform to your admin.");
                throw new GeneralBOException("Error, Found problem when getting branch, please inform to your admin.");
            }

            String stTanggal = CommonUtil.convertDateToString( new java.util.Date());

            reportParams.put("payrollHeaderId",payrollHeaderId);
            reportParams.put("areaId",CommonUtil.userAreaName());
            reportParams.put("urlLogo", CommonConstant.URL_LOGO_REPORT + branch.getLogoName());
            reportParams.put("branchId", branchId);
            reportParams.put("branchName", branch.getBranchName());
            ...
            logger.error("[ReportAction.printReportPayrollByBranch] Error when print slip payroll not found branch id, Found problem when downloading data, please inform to your admin.");
            throw new GeneralBOException("Error, Found problem when print slip payroll not found branch id, please inform to your admin.");
        }
