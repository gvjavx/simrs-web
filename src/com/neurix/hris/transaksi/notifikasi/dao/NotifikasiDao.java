...
public class NotifikasiDao extends GenericDao<ImNotifikasiEntity, String> {
    ...
    public List<ImNotifikasiEntity> getByCriteriaApproval(Map mapCriteria) {
        ...
        String query = "SELECT DISTINCT  \n" +
                "                                                data.*,   \n" +
                "                                                case    \n" +
                "                                                when data.tipe_notif_id = 'TN77' AND data.lembur is null     \n" +
                "                                                then 'L'   \n" +
                "                                                when data.tipe_notif_id = 'TN66' AND data.cuti is null     \n" +
                "                                                then 'C'   \n" +
                "                                                when data.tipe_notif_id = 'TN88' AND data.ijin_keluar is null     \n" +
                "                                                then 'I'   \n" +
                "                                                when data.tipe_notif_id = 'TN55' AND data.dispensasi is null     \n" +
                "                                                then 'D'   \n" +
                "                                                when data.tipe_notif_id = 'TI' AND data.sppd is null     \n" +
                "                                                then 'S'   \n" +
                "                                                when data.tipe_notif_id = 'TN33' AND data.absensi is null     \n" +
                "                                                then 'A'   \n" +
                "                                                when data.tipe_notif_id = 'TN99' AND data.rekruitmen is null     \n" +
                "                                                then 'R' \n" +
                "                                                when data.tipe_notif_id = 'TN23' AND data.training is null     \n" +
                "                                                then 'T' \n" +
                "                                                when data.tipe_notif_id = 'TN23' AND data.trainingbos is null     \n" +
                "                                                then 'B' \n" +
                "                                                when data.tipe_notif_id = 'TN44' AND data.indisipliner is null     \n" +
                "                                                then 'P'\n" +
                "                                                end as hasil   \n" +
                "                                                from   \n" +
                "                                                (select   \n" +
                ...
                "                                                from   \n" +
                "                                                ( SELECT * FROM it_hris_notifikasi WHERE flag='Y' AND tipe_notif_id<>'umum' AND tipe_notif_id<>'SPPD') notif    \n" +
                "                                                left join ( SELECT * FROM it_hris_lembur ) lembur on lembur.lembur_id = notif.no_request   \n" +
                "                                                left join ( SELECT * FROM it_hris_cuti_pegawai ) cuti on cuti.cuti_pegawai_id = notif.no_request   \n" +
                "                                                left join ( SELECT * FROM it_hris_ijin_keluar WHERE ijin_Id='IJ001') ijin_keluar on ijin_keluar.ijin_keluar_id = notif.no_request   \n" +
                "                                                left join ( SELECT * FROM it_hris_ijin_keluar WHERE ijin_Id<>'IJ001') dispensasi on dispensasi.ijin_keluar_id = notif.no_request   \n" +
                "                                                left join ( SELECT * FROM it_hris_sppd_person) sppd on sppd.sppd_id = notif.no_request AND sppd.person_id=notif.from_person   \n" +
                "                                                left join ( SELECT * FROM it_hris_absensi_pegawai) absensi on absensi.absensi_pegawai_id = notif.no_request \n" +
                "                                                left join ( SELECT * FROM it_hris_training_person) training on training.training_id = notif.no_request \n" +
                "                                                left join ( SELECT * FROM it_hris_rekruitmen_pabrik_detail) rekruitmen on rekruitmen.rekruitmen_pabrik_id = notif.no_request\n" +
                "                                                left join ( SELECT * FROM it_hris_indisipliner) indisipliner on indisipliner.indisipliner_id = notif.no_request  \n" +
                "                                                ) as data   \n" +
                "                                                   \n" +
                "                                                WHERE nip='"+nip+"'  \n" +
