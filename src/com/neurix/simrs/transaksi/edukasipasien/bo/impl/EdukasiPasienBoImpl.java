...
public class EdukasiPasienBoImpl implements EdukasiPasienBo {
        ...
        if (bean != null) {
            ...
            if (entityList.size() > 0) {
                for (ItSimrsEdukasiPasienEntity entity : entityList) {
                    ...
                    edukasiPasien.setNamaTerang(entity.getNamaTerang());
                    edukasiPasien.setSip(entity.getSip());
                    edukasiPasien.setNamaStaf(entity.getNamaStaf());
    ...
    public CrudResponse saveAdd(List<EdukasiPasien> list) throws GeneralBOException {
        ...
        if (list.size() > 0) {
            EdukasiPasien edukasi= list.get(0);
            EdukasiPasien eduks = new EdukasiPasien();
            eduks.setIdDetailCheckup(edukasi.getIdDetailCheckup());
            eduks.setKeterangan(edukasi.getKeterangan());
            List<EdukasiPasien> edukasiPasienList = getByCriteria(eduks);

            if(edukasiPasienList.size() > 0){
