...
<head>
    ...
    <script type='text/javascript'>
        ...
        $.subscribe('beforeProcessSave1', function (event, data) {
            ...
            var unit2       = $("#branchId option:selected").val();
            ...
            var totalBobot = document.getElementById("totalBobot").value;
            var totalSeluruhAspek = Number(totalAspekA) +Number(totalAspekB) + Number(totalAspekC) + Number(totalBobot);

            if(totalSeluruhAspek <= 100){
                if (unit != '' && jabatan != '' && tipeAspek != '') {
                    if(cekSudahTerinput(unit2, jabatan, tipeAspek) == '0'){
                        ...
                        event.originalEvent.options.submit = false;
                        var msg = "";
                        msg += '<strong> Data Aspek sudah terinput</strong>' + '<br/>';
                            ...
                            <tr>
                                ...
                                <td>
                                    <table>
                                        ...
                                        <s:if test="isAddOrEdit()">
                                            <s:textfield cssStyle="display: none" id="tmpJabatan" name="smkJabatan.positionName"></s:textfield>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form class="form-horizontal" id="myForm">
                    ...
                    <div id="satuan" class="form-group">
...
<script>
    window.loadSubTipe =  function(){
        ...
        SmkJabatanAction.searchSmkJabatanSubTipeA(function(listdata) {
            ...
            tmp_table = "<thead style='font-size: 14px; color: white' ><tr class='active'>"+
                    ...
                    "<th style='text-align: center; background-color:  #3c8dbc''>Satuan</th>"+
                    "</tr></thead>";
            </s:if>
            <s:else>
            tmp_table = "<thead style='font-size: 14px; color: white' ><tr class='active'>"+
            ...
            <s:if test="isAddOrEdit()">
            tmp_table += '<tfoot><tr><td colspan="3" style="font-size: 12px;text-align: center">Jumlah Bobot' +
            ...
            $.each(listdata, function (i, item) {
                <s:if test="isAddOrEdit()">
                    <s:if test="isAdd()">
                        tmp_table += '<tr style="font-size: 12px;" ">' +
                                '<td align="center">' +
                                "<a href='javascript:;' class ='item-editBC' data ='"+item.jabatanSmkDetailId+"' >" +
                                "<img border='0' src='<s:url value='/pages/images/icon_edit.ico'/>' name='icon_edit'>"+
                                '</a>' +
                                '<td>' + item.indikatorKinerja + '</td>' +
                                '<td align="center">' + item.bobot+ '</td>' +
    ...
    window.cekJabatan = function(id){
        ...
        SmkJabatanAction.getNilaiAspek(branchId, id.value, function(listdata) {
            totalAspekA = listdata.totalAspekA;
            totalAspekB = listdata.totalAspekB;
            totalAspekC = listdata.totalAspekC;
        });
        $('#tmpAspekA').val(totalAspekA);
        $('#tmpAspekB').val(totalAspekB);
        $('#tmpAspekC').val(totalAspekC);
        //$('#tmpAspekA').val(hasil);
        if(tipeAspek == 'A'){
            loadSubTipe();
            //alert('aspek A');
    ...
    window.cekSudahTerinput = function(unit, jabatan, aspek){
        var hasil = "-";

        SmkJabatanAction.cekSudahTerinput(unit, jabatan, aspek, function(listdata) {
           hasil = listdata;
        });
    ...
    $(document).ready(function() {
        ...
        $('.smkSubTipeTable').on('click', '.item-editBC', function(){
            ...
            SmkJabatanAction.searchSmkJabatanDetail(id,function(listdata) {
                $.each(listdata, function (i, item) {
                    $('#kategoriId').val(item.jabatanSmkDetailId);
                    $('#subTipeNama').val(item.indikatorKinerja);
                    $('#subTipeNamaOld').val(item.indikatorKinerja);
                    $('#subTipeBobot').val(item.bobot);
                });
        ...
        $('#btnSave').click(function(){
            ...
            if(url == 'addSubTipe'){
                ...
                if(subTipeNama != ''){
                    dwr.engine.setAsync(false);
                    SmkJabatanAction.saveEditSmkJabatan(subTipeNamaOld, subTipeNama, subTipeBobot, "", "", subTipeSatuan, function(listdata) {
                        $('#modal-smkJabatan').modal('hide');
                        $('#myForm')[0].reset();
        ...
        <s:if test="isAddOrEdit()">
            var aspekId   = document.getElementById("tipeAspekId");
            <s:if test="isAdd()">
                //alert('add');
            </s:if>
            <s:else>
    ...
    window.listPosisi = function(branch, divisi){
        ...
        var tmpJabatan = document.getElementById("tmpJabatan").value;
        $('#positionId').empty();
        ...
        if(tmpJabatan != ''){
            //alert(tmpJabatan);
            $('#positionId').val(tmpJabatan);
        }
        var posisi = document.getElementById("positionId");
        <s:if test="isAdd()">
            cekJabatan(posisi);
        </s:if>
        <s:else>
        ...
        $('#tmpAspekB').val(0);
        $('#tmpAspekC').val(0);
            SmkJabatanAction.getNilaiAspek(branch, posisi.value, function(listdata) {
                if(tipeAspek == "A"){
                    $('#tmpAspekB').val(listdata.totalAspekB);
                    $('#tmpAspekC').val(listdata.totalAspekC);
                }else if(tipeAspek == "C"){
                    $('#tmpAspekA').val(listdata.totalAspekA);
                    $('#tmpAspekB').val(listdata.totalAspekB);
                }else {
                    $('#tmpAspekA').val(listdata.totalAspekA);
                    $('#tmpAspekC').val(listdata.totalAspekC);
